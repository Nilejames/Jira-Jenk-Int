...
def call(Map config = [:]) {
    ...
    def response = sh(
        script: 'curl -D- -s -o response.json -w "%{http_code}"  -H "Authorization: Bearer "$JIRA_CREDENTIALS -X POST --data "'+render+'" -H "Content-Type: application/json" $JIRA_URL/rest/api/2/issue',
        returnStdout: true
    ).trim()
